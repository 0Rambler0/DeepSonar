...
def vggvox_resnet2d_icassp(input_dim=(257, 250, 1), num_class=8631, mode='train', args=None):
    ...
    if net == 'resnet34s':
        inputs, x, activation_data= backbone.resnet_2D_v1(input_dim=input_dim, mode=mode)
    else:
        inputs, x, activation_data = backbone.resnet_2D_v2(input_dim=input_dim, mode=mode)
    activation_matrix.extend(activation_data)
    # ===============================================
    ...
    model = keras.models.Model(inputs, activation_matrix, name='vggvox_resnet2D_{}_{}'.format(loss, aggregation))
